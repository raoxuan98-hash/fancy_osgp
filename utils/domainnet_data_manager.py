...
from utils.domainnet_data import iGanFake, iCore50, iDomainNet, iOfficeHome
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args=None):
        ...
        for _ in range(self.nb_tasks - 1):
    ...
    @property
    def nb_tasks(self):
        return self.args["total_sessions"]
    ...
    def get_dataset(self, indices, source, mode, appendent=None, ret_data=False):
        """
        @ param indices: list; classes to be selected; e.g. [0, 1, ..., 344] on DomainNet
        @ param mode: train, flip, test
        @ param appendent: (data, targets)
        """
        if source == 'train':
    ...
    def get_anchor_dataset(self, mode, appendent=None, ret_data=False):
        """
        NB: useless here; only used in triplet-based CIL methods, e.g. R-DFCIL
        """
        if mode == 'train':
            trsf = transforms.Compose([*self._train_trsf, *self._common_trsf])
        elif mode == 'flip':
        ...
        if appendent is not None:
            ...
            for idx in range(0, int(np.max(appendent_targets))+1):
                ...
                try:
                    ...
                    logging.info('Not enough samples for class {} in appendent data, allowing replacement.'.format(idx))
                    val_indx = np.random.choice(len(append_data), len(append_data), replace=True)
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.use_path:
           ...
           try:
                image = self.trsf(Image.fromarray(self.images[idx].astype(np.uint8)))
           except:
                print("Something wrong with image value: ", self.images[idx])
