...
class IncrementalDataManager:
    ...
    def get_dataset(
        ...
        data, targets = self._select_by_classes(source, list(class_indices))
        trsf = self._build_transform(mode)
        ds = _DummyIndexedDataset(data, targets, trsf, self.use_path)
        return ds


class _DummyIndexedDataset(Dataset):
    def __init__(self, images, labels, transform, use_path: bool = False) -> None:
    ...
    def __getitem__(self, idx):
        if self.use_path:
            # images holds file paths when use_path is True
            with open(self.images[idx], "rb") as f:
                img = Image.open(f).convert("RGB")
        else:
            img = Image.fromarray(self.images[idx].astype(np.uint8))
        img = self.transform(img)
        return idx, img, int(self.labels[idx])
