...
def set_smart_defaults(ns):
    ...
    if ns.dataset == 'cars196_224':
        ...
        ns.init_cls, ns.increment, ns.epochs = 20, 20, 15
    elif ns.dataset == 'domainnet':
        # 如果用户没显式给 data_path，则使用项目中的默认路径
        if not getattr(ns, "data_path", ""):
            ns.data_path = '/home/raoxuan/projects/SubspaceLoRA/data/DomainNet'
         # 不强制覆盖 init_cls/increment，保持 parser 的默认值或用户指定的值
        ns.class_incremental = True   # <- 新增：domainnet 默认用 class_incremental 模式
    # 修正：使用 ns（不是未定义的 args）    
...
# --------------------------------------------------------------
def main(args):
    """把已经解析好的 ``Namespace``（dict） 交给 trainer。"""
    train(args)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--dataset', type=str, default='domainnet',
                       choices=['imagenet-r', 'cifar100_224', 'cub200_224', 'cars196_224', 'domainnet'],
                       help='Dataset to use')
    basic.add_argument('--data_path', type=str, default='/home/raoxuan/projects/SubspaceLoRA/data/DomainNet',
                       help='Root path for datasets (for domainnet set to e.g. /home/raoxuan/projects/SubspaceLoRA/data/DomainNet)')
    ...
    basic.add_argument('--user', type=str, default='simian_domainnet', choices=['null'], help='User identifier (currently unused).')
    ...
    cls.add_argument('--init_cls', type=int, default=345, help='Number of classes in the first task.')
    cls.add_argument('--increment', type=int, default=345, help='Number of new classes added per task.')
    cls.add_argument('--class_incremental', action='store_true', default=True,
                 help='If set, DomainNet will be handled in class-incremental mode.')
    ...
    model.add_argument('--vit_type', type=str, default='vit-b-p16-mocov3',
    ...
    train_grp.add_argument('--epochs', type=int, default=5, help='Training epochs per task.')
    ...
    reg.add_argument('--l2_protection_lambda', type=float, default=1e-3, help='Weight for the L2-protection term (higher → stronger regularisation). When `--l2_protection` is off, this will be automatically set to 0.0.')
