...
def set_smart_defaults(ns):
    ...
    if ns.dataset == 'cars196_224':
        ...
        ns.init_cls, ns.increment, ns.iterations = 20, 20, 2500
    elif ns.dataset == 'cifar100_224':
        ns.init_cls, ns.increment, ns.iterations = 10, 10, 2500
...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--smart_defaults', action='store_true', default=True, help='If set, overwrite a few hyper‑parameters according to the dataset.')
    basic.add_argument('--user', type=str, default='authors', choices=['null'], help='User identifier (currently unused).')
    ...
    model.add_argument('--weight_temp', type=float, default=5.0, help='Projection temperature.')
    ...
    train_grp.add_argument('--iterations', type=int, default=20, help='Training iterations per task.')
    ...
    train_grp.add_argument('--optimizer', type=str, default='sgd', help='Optimizer name (adamw / sgd).')
    train_grp.add_argument('--lrate', type=float, default=2e-3, help='Learning rate.')
    ...
    train_grp.add_argument('--kd_type', type=str, default='feat')
