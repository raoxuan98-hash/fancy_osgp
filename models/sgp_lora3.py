...
# ================= SGPLinear：带可训练 scales 的 LoRA（A/B 仅调控方向） =================
class SGPLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # === A 仅表示方向 → 按行归一化（每行是低秩方向向量）===
        A_norm = self.A / (self.A.norm(dim=1, keepdim=True) + 1e-8)  # (r, in_features)
        A_eff_base = A_norm @ P_basis  # (r, d)
        A_eff = A_eff_base * self.scales.unsqueeze(0)  # 应用可学习缩放
        h = F.linear(x, A_eff)

        # === B 仅表示方向 → 按列归一化（每列是输出方向向量）===
        B_norm = self.B / (self.B.norm(dim=0, keepdim=True) + 1e-8)  # (out_features, r)
        lora_update = F.linear(h, B_norm)
...
# ================= SGPQKV：用于 Attention QKV 的适配器（A/B 仅调控方向） =================
class SGPQKV(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # === A 仅表示方向 → 按行归一化 ===
        A_norm = self.A / (self.A.norm(dim=1, keepdim=True) + 1e-8)
        A_eff_base = A_norm @ P_basis
        A_eff = A_eff_base * self.scales.unsqueeze(0)
        h = F.linear(x, A_eff)

        # === B 仅表示方向 → 按列归一化 ===
        B_norm = self.B / (self.B.norm(dim=0, keepdim=True) + 1e-8)
        lora_update = F.linear(h, B_norm)
...
class BaseLoRAViT(nn.Module):
    ...
    def reset_parameters_svd(self) -> None:
        for module in self.lora_modules.values():
            if hasattr(module, 'target_weight'):
                ...
                # 初始化 A 为前 r 个右奇异向量，并归一化方向
                A_init = Vh[:self.r, :]
                module.A.data = A_init / (A_init.norm(dim=1, keepdim=True) + 1e-8)
                # B 初始化为零（方向任意，幅度为0）
                module.B.data.zero_()
    ...
    def regularization_loss(self) -> torch.Tensor:
        ...
        return self.scales_reg_weight * total_loss if hasattr(self, 'scales_reg_weight') else total_loss
        ...
        with torch.no_grad():
            for name, module in self.lora_modules.items():
                try:
                    ...
                    diff = s - si
                    k = min(50, len(diff))
                    topk_diff_mean = torch.topk(diff, k=k, largest=True).values.mean().item()
                    botk_diff_mean = torch.topk(diff, k=k, largest=False).values.mean().item()
                    ...
                    print(f"[DEBUG] {name} | Δ top{k} mean: {topk_diff_mean:+.4f} | Δ bot{k} mean: {botk_diff_mean:+.4f} | Spearman ρ: {rho:.4f}")

                except Exception as e:
                    print(f"[Warning] Spearman failed for {name}: {e}")
                    rho = 0.0
