...
def compute_weighted_covariances(stats_dict: Dict[int, object], temperature: float = 1.0, device: str = "cpu"):
    ...
    covs = torch.stack([stats_dict[cid].cov.to(device) for cid in class_ids])
    means_norm = F.normalize(means, dim=1)
    sim_matrix = torch.mm(means_norm, means_norm.t())
    weights = F.softmax(sim_matrix / temperature, dim=1)
    weighted_covs = torch.einsum('ij,jkl->ikl', weights, covs)
    stats_dict = {cid: GaussianStatistics(means[i], weighted_covs[i], reg=stats_dict[cid].reg) for i, cid in enumerate(class_ids)}
...
class Drift_Compensator(object):
    def __init__(self, args):
        ...
        self.auxiliary_data_size = args.get('auxiliary_data_size', 512)
        self.args = args
    ...
    def compute_nonlinear_transform(self, features_before: torch.Tensor, features_after: torch.Tensor, normalize: bool = False):
        device = "cuda:0"
        features_before = features_before.to(device)
        features_after = features_after.to(device)
        ...
        dataset = TensorDataset(features_before, features_after)
        ...
        for i in range(1, iterations + 1):
            ...
            optimizer.zero_grad()
            y_pred = mlp(x)
            loss = F.mse_loss(y_pred, y) + 0.1*mlp.reg_loss()
            loss.backward()
            optimizer.step()
            scheduler.step()
        ...
        mlp.eval()
        with torch.no_grad():
            features_pred = mlp(features_before)
            norm_error_before = (features_before - features_after).norm(dim=1).mean().item()
            norm_error_pred = (features_pred - features_after).norm(dim=1).mean().item()
        print(f"非线性修正前差异：{norm_error_before:.4f}；修正后差异：{norm_error_pred:.4f}")
        return mlp
    
    @torch.no_grad()
    def transform_stats_with_mlp(self, stats_dict, mlp):
        device = "cuda:0"
        mlp.to(device)
        cached_eps = None
        new_stats_dict = {}
        for key, stat in stats_dict.items():
            if cached_eps is None:
                feature_dim = stat.mean.shape[-1]
                num_cached_eps = 5 * feature_dim
                cached_eps = torch.randn(num_cached_eps, feature_dim)
            samples = stat.sample(cached_eps=cached_eps).to(device)
            updated_samples = mlp(samples)
            mean = updated_samples.mean(dim=0).squeeze().cpu()
            cov = torch.cov(updated_samples.T).cpu()
            new_stats_dict[key] = GaussianStatistics(mean, cov)
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        if not hasattr(self, 'variants') or len(self.variants) == 0:
            self.variants = {
                "per-class": {},
                "per-class_compensate": {},
                "per-class_compensate_mlp": {}}  # <-- 新增 MLP 补偿分支

        # 更新原始分布
        self.variants["per-class"].update(stats)

        # 线性补偿更新
        if self.compensate and task_id > 0 and W is not None:
            stats_compensated = self.variants['per-class_compensate']
            stats_compensated = self.transform_stats_with_W(stats_compensated, W)
        ...
        # MLP 补偿更新（新增）
        if self.compensate and task_id > 0:
            # 训练 MLP 变换器（使用辅助数据 + 当前任务数据）
            feats_b_mlp = torch.cat([feats_before, feats_aux_before], dim=0)
            feats_a_mlp = torch.cat([feats_after, feats_aux_after], dim=0)
            mlp = self.compute_nonlinear_transform(feats_b_mlp, feats_a_mlp)

            # 对历史分布做非线性变换
            stats_compensated_mlp = self.variants['per-class_compensate_mlp']
            stats_compensated_mlp = self.transform_stats_with_mlp(stats_compensated_mlp, mlp)
            self.variants["per-class_compensate_mlp"] = stats_compensated_mlp
            self.variants["per-class_compensate_mlp"].update(copy.deepcopy(stats))
        else:
            self.variants["per-class_compensate_mlp"].update(copy.deepcopy(stats))

        # 构建加权协方差变体（复用原逻辑，自动包含新分支）
        new_variants = {}
        for key, stats in self.variants.items():
            if "weighted_covariance" not in key:
                new_key = f"{key}-weighted_covariance"
                new_stats = compute_weighted_covariances(copy.deepcopy(stats), 0.1)
                new_variants[new_key] = new_stats
        self.variants.update(new_variants)

        print(f"[INFO] Built distribution variants: {list(self.variants.keys())}")
